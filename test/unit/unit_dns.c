...
START_TEST (test_dns)
{
    ...
    ret = pico_dns_client_nameserver(stack, NULL, 99);
    fail_if(ret == 0, "dns> dns_client_nameserver wrong code");

    ret = pico_dns_client_nameserver(stack, NULL, 0xFF);
    fail_if(ret == 0, "dns> dns_client_nameserver wrong code");

    ret = pico_dns_client_nameserver(stack, &ns, PICO_DNS_NS_DEL); /* delete non added ns */
    fail_if(ret == 0, "dns> dns_client_nameserver del error");

    ret = pico_dns_client_nameserver(stack, &ns, 99);
    fail_if(ret == 0, "dns> dns_client_nameserver wrong code");

    ret = pico_dns_client_nameserver(stack, &ns, PICO_DNS_NS_ADD); /* add correct one */
    fail_if(ret < 0, "dns> dns_client_nameserver add error: %s", strerror(pico_err));

    ret = pico_dns_client_nameserver(stack, &ns, 99);
    fail_if(ret == 0, "dns> dns_client_nameserver wrong code");

    ret = pico_dns_client_nameserver(stack, &ns, PICO_DNS_NS_DEL);
    fail_if(ret < 0, "dns> dns_client_nameserver del error: %s", strerror(pico_err));

    ret = pico_dns_client_nameserver(stack, &ns, PICO_DNS_NS_ADD); /* add correct one */
    fail_if(ret < 0, "dns> dns_client_nameserver add error: %s", strerror(pico_err));

    ret = pico_dns_client_nameserver(stack, &ns, PICO_DNS_NS_ADD); /* add correct one again */
    ...
    ret = pico_dns_client_getaddr(stack, NULL, cb_dns, NULL);
    fail_if(ret == 0, "dns> dns_client_getaddr: no url");

    ret = pico_dns_client_getaddr(stack, url, NULL, NULL);
    ...
    ret = pico_dns_client_getname(stack, NULL, cb_dns, NULL);
    fail_if(ret == 0, "dns> dns_client_getname: no ip");

    ret = pico_dns_client_getname(stack, ip, NULL, NULL);
