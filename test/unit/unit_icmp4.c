...
START_TEST (test_icmp4_ping)
{
    ...
    fail_if(pico_icmp4_ping(S, local_address, NUM_PING, interval, timeout, size, cb_ping) < 0);
    pico_stack_tick(S);
    pico_stack_tick(S);
    pico_stack_tick(S);
    ...
    pico_icmp4_ping(S, remote_address, NUM_PING, interval, timeout, size, cb_ping);
    pico_stack_tick(S);
    pico_stack_tick(S);
    pico_stack_tick(S);
...
START_TEST (test_icmp4_unreachable_send)
{
    ...
    fail_if(pico_icmp4_dest_unreachable(S, f));
    pico_stack_tick(S);
    pico_stack_tick(S);
    pico_stack_tick(S);
    ...
    fail_if(pico_icmp4_port_unreachable(S, f));
    pico_stack_tick(S);
    pico_stack_tick(S);
    pico_stack_tick(S);
    ...
    fail_if(pico_icmp4_proto_unreachable(S, f));
    pico_stack_tick(S);
    pico_stack_tick(S);
    pico_stack_tick(S);
    ...
    fail_if(pico_icmp4_ttl_expired(S, f));
    pico_stack_tick(S);
    pico_stack_tick(S);
    pico_stack_tick(S);
...
START_TEST (test_icmp4_unreachable_recv)
{
    ...
    /* open a socket */
    sock = pico_socket_open(S, PICO_PROTO_IPV4, PICO_PROTO_TCP, &icmp4_unreach_socket_cb);
    fail_if(sock == NULL);
