...
START_TEST (arp_lookup_test)
{
    ...
    eth = pico_arp_lookup(S, &ip);
    fail_unless(eth == NULL);
    pico_tree_delete(&S->arp_tree, &entry);
}
END_TEST
...
START_TEST (arp_receive_test)
{
    ...
    fail_if(pico_ipv4_link_add(S, mock->dev, ip1, netmask), "add link to mock device failed");
    ...
    ah->s_mac[0] = 0x01;
    fail_unless(pico_arp_receive(f) == -1);
    pico_ipv4_link_del(S, mock->dev, ip1);
}
END_TEST
...
START_TEST (arp_get_test)
{
    ...
    mock = pico_mock_create(S, macaddr);
    fail_if(!mock, "MOCK DEVICE creation failed");
    fail_if(pico_ipv4_link_add(S, mock->dev, ip, netmask), "add link to mock device failed");
    ...
    eth = pico_arp_get(S, f);
    fail_unless(eth == &mock->dev->eth->mac);
    pico_ipv4_link_del(S, mock->dev, ip);
}
END_TEST
